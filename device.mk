...
# audio_policy_configuration.xml lives in etc/audio/sku_volcano and pulls these
# three in with <xi:include href="...">, which resolves relative to the including
# file. Installing them only under etc/ meant none of the includes resolved, so no
# volume curves were loaded at all and the volume keys did nothing. Stock ships a
# copy in each sku directory for exactly this reason.
PRODUCT_COPY_FILES += \
    frameworks/av/services/audiopolicy/config/r_submix_audio_policy_configuration.xml:$(TARGET_COPY_OUT_VENDOR)/etc/audio/sku_volcano/r_submix_audio_policy_configuration.xml \
    $(LOCAL_PATH)/audio/audio_policy_volumes.xml:$(TARGET_COPY_OUT_VENDOR)/etc/audio/sku_volcano/audio_policy_volumes.xml \
    $(LOCAL_PATH)/audio/default_volume_tables.xml:$(TARGET_COPY_OUT_VENDOR)/etc/audio/sku_volcano/default_volume_tables.xml \
